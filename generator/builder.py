...
{chr(10).join(items)}
    ...
    def build(self):
        """Build the site."""
        self.scan_content()
        travel_data = self.load_travel_data() if CONFIG.get("enable_travel") else []
        talks_data = self.load_talks_data() if CONFIG.get("enable_talks") else []
